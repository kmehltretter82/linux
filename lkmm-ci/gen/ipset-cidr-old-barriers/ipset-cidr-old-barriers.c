...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && count[_i] != 0) fatal("ipset-cidr-old-barriers, check_globals failed");
    if (rand_bit(&(_a->seed)) && cidr[_i] != 0) fatal("ipset-cidr-old-barriers, check_globals failed");
  }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ipset-cidr-old-barriers Allowed\n");
  ...
  fprintf(out,"Hash=cb5dde9010d50efa7f79e98a2f6e0ec9\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ipset-cidr-old-barriers %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ipset-cidr-old-barriers %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ipset-cidr-old-barriers: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ipset-cidr-old-barriers, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
